...
export class SignInDriverUseCase {
  constructor(private driversRepository: DriversRepository, private jwtService: JwtService) { }
  ...
  async execute({ email, password }: { email: string; password: string }) {
    ...
    const token = this.jwtService.sign({ userId: driver.id, email: driver.email });
    return { driver, token };
  }
}
